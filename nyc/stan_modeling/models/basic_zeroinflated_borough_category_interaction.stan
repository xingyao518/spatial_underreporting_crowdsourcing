...
data {// Define variables in data
  ...
  int<lower=0> N_borough_category; // number of interaction terms
  matrix[N_incidents, N_borough_category] X_borough_category; // interaction terms design matrix
...
transformed data {
    ...
    matrix[N_incidents, 1 + 5 +covariate_matrix_width+N_category+N_borough_category] X_total; 
    X_total = append_col(append_col(append_col(append_col(ones, X_borough_category), X_borough), X_category), X);
...
parameters {
  ...
  matrix[4, N_category-1] beta_borough_category_raw;
...
transformed parameters  {
    ...
    vector[N_borough_category] beta_borough_category;
    vector[1 + 5 + covariate_matrix_width + N_category + N_borough_category] beta_total;
    ...
    beta_borough_category = to_vector(beta_borough_category_raw); // not sure this is correct
    

    beta_total = append_row(append_row(append_row(append_row(intercept, beta_borough_category), beta_borough), beta_category), beta);
...
model {
  // Prior part of Bayesian inference
    ...
    beta_borough_category ~ normal(0, 1*inv(sqrt(1 - inv(N_borough_category)))); //sigma_borough
