...
transformed data {
    ...
    matrix[N_incidents, 1 + 5 +covariate_matrix_width+N_category + N_month] X_total; 
    X_total = append_col(append_col(append_col(append_col(ones, X_borough), X_category), X), X_month); // add month here
...
parameters {
  ...
  vector[N_month] beta_month; // new betas for month
...
transformed parameters  {
    ...
    beta_total = append_row(append_row(append_row(append_row(intercept, beta_borough), beta_category), beta), beta_month);
...
model {
  // Prior part of Bayesian inference
    ...
    beta_month[1] ~ normal(0, 5);
    beta_month[2:N_month] ~ normal(beta_month[1:(N_month - 1)], 0.05); // AR prior for month
