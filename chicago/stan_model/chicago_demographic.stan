...
transformed data {
    ...
    // matrix[N_incidents, 1 + N_tract +covariate_matrix_width + N_category] X_total; 
    // X_total = append_col(append_col(append_col(ones, X_tract), X_category), X);
    matrix[N_incidents, 1 + covariate_matrix_width + N_category] X_total; 
    X_total = append_col(append_col(ones, X_category), X);
}
...
transformed parameters  {
    ...
    // vector[1 + N_tract + covariate_matrix_width + N_category] beta_total;
    vector[1 + covariate_matrix_width + N_category] beta_total;
    ...
    // beta_total = append_row(append_row(append_row(intercept, beta_tract), beta_category), beta);
    beta_total = append_row(append_row(intercept, beta_category), beta);
...
model {
  // Prior part of Bayesian inference
    ...
    // target += -0.5 * dot_self(beta_tract[node1] - beta_tract[node2]);
}
