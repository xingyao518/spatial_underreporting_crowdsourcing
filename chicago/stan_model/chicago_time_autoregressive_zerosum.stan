...
data {// Define variables in data
  ...
  // time data
  int<lower=0> N_month;
  matrix[N_incidents, N_month] X_month;
}
...
transformed data {
    ...
    // matrix[N_incidents, 1 + N_tract +covariate_matrix_width + N_category + N_month] X_total; 
    // X_total = append_col(append_col(append_col(append_col(ones, X_tract), X_category), X), X_month); // add X_month

    matrix[N_incidents, 1 + N_category + N_month] X_total; 
    X_total = append_col(append_col(ones, X_category), X_month); // add X_month
...
parameters {
  ...
  vector[N_month-1] beta_month_raw; // new betas for month
}
...
transformed parameters  {
    ...
    // vector[1 + N_tract + covariate_matrix_width + N_category + N_month] beta_total;
    vector[1 + N_category + N_month] beta_total;

    beta_category = append_row(beta_category_raw, -sum(beta_category_raw));
    beta_month = append_row(beta_month_raw, -sum(beta_month_raw));
    ...
    beta_total = append_row(append_row(intercept, beta_category),beta_month); // add beta_month
...
model {
  // Prior part of Bayesian inference
    ...
    beta_month[1] ~ normal(0, 0.01);
    beta_month[2:N_month] ~ normal(beta_month[1:(N_month - 1)], 0.025); // AR prior for month
