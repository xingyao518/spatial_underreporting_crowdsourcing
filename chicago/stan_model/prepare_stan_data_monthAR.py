...
def get_edge_lists_for_tract_adjacency(aggdftractorder, adjacency_file = 'data/tract_adjacency.npz', tract_adjacency_order_file = 'data/tract_order_adjacency'):
...
def get_data_dictionary(aggdf, maxdatapoints=None, covariates_cont=None, tract11_or_block12 = 11):
    column_names = {}  # used for plotting (saving the order of the columns)
    covariates = ['OWNER_DEPARTMENT']  # ['Category']
    ...
    # deleted Borough here
    # colstodo = ['NumberDuplicates', 'Duration'] + ['Borough'] + covariates
    colstodo = ['NumberDuplicates', 'Duration'] + covariates
    ...
    # category renamed to SR_SHORT_CODE
    _, X_category_dm = dmatrices(
        'NumberDuplicates ~ 0 + SR_SHORT_CODE', data=aggdfloc)
    ...
    data = {"N_incidents": len(y), "y": y, "duration": duration, "X": X, "X_category": X_category,
    ...
    _, X_month_dm = dmatrices(
        'NumberDuplicates ~ 0 + CreatedDate', data=aggdfloc)
    X_month = np.asarray(X_month_dm)

    column_names['Month'] = X_month_dm.design_info.column_names
    data.update({"X_month": X_month, "N_month": np.shape(X_month)[1]})
