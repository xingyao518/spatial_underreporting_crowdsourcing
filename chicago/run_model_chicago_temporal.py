...
from stan_model.prepare_stan_data_monthAR import *
...
def run_model(modelname, 
        ...
        tract11_or_block12 = 12, earliest_time = None, latest_time = None):
    ...
    ## load and prepare data
        ...
        if earliest_time is not None:
            aggdf = aggdf.query('CreatedDate >= @earliest_time')
        if latest_time is not None:
            aggdf = aggdf.query('CreatedDate <= @latest_time')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", type = str, default = 'chicago_time_autoregressive_zerosum')
    parser.add_argument("--iter_warmup", type=int, default=50)
    parser.add_argument("--iter_sampling", type=int, default=100)
    parser.add_argument("--max_incidents", type=int, default=1000)
    parser.add_argument("--save_label", type=str, default=os.times().system)
    parser.add_argument("--earliest_time", type=str, default=None)
    parser.add_argument("--latest_time", type=str, default=None)
    parser.add_argument("--settings_name", type=str, default=None)

    args = parser.parse_args()

    run_model(modelname = args.model_name, iter_warmup=args.iter_warmup, iter_sampling=args.iter_sampling, max_incidents=args.max_incidents, save_label=args.save_label, earliest_time = args.earliest_time, latest_time = args.latest_time, settings_name=args.settings_name, show_progress=True)